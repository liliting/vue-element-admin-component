  ...
  <template class="sidebarItem-container" v-if="!item.hidden&&item.children">
    ...
    <el-submenu v-else-if="!item.hidden" :index="item.name+'/'+item.path">
      ...
      <template v-for="child in item.children">
        <sidebar-item v-if="child.children&&child.children.length>0" :item="child" :key="child.path" :base-path="child.path"></sidebar-item>
        <router-link v-else class="item-links" :to="item.path+'/'+child.path" :key="child.name">
